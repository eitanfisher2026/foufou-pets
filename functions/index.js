...
Not all identifying features are equally reliable. Coat color and pattern are the strongest signal - they stay visible and comparable even through blur, a bad angle, or different lighting, and two individuals of the same species/breed only rarely share the exact same coat. Distinctive markings are the next most reliable. Body/face/ear shape and general build are the weakest signal - they vary enormously with breed, pose, angle, and how filled-out or thin an animal looks in a given photo, and many unrelated animals of a similar breed or size share a similar silhouette. A shared build or a similarly-shaped body is NOT enough on its own to call two animals a plausible match - if the coat color/pattern is visible in both photos and clearly does not match, that alone should pull your verdict down to "low" or "noMatch" even when the build looks similar, not up to "medium". Reserve "medium"/"high" for cases where the coat itself genuinely supports a match (or is genuinely too unclear in BOTH photos to judge at all) - not for "different coats, but similar-looking dogs/cats overall".
...
- "medium": plausibly the same animal, but with real uncertainty (e.g. the coat itself is too unclear in both photos to judge, or only partial visual evidence).
- "low": more likely different than the same - a visible coat color/pattern difference that isn't stark enough to be certain, weak or partial conflicting evidence, OR the photos are too poor/limited (blurry, animal not clearly visible, very different framing) to compare with any real confidence either way. Nothing here definitively rules it out, but there's no real basis to call it a match.
