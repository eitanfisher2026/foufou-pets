...
- **Auto-approve the routine dev loop, no stopping to confirm.** Eitan runs with auto-mode on specifically so this doesn't happen. This explicitly covers, without pausing: `npm run build`, `firebase deploy`, `git add`/`commit`/`push` on this repo's own `main` branch, and the full browser-verification harness cycle (temporarily swapping `src/main.jsx` to render `TestHarness.jsx` behind `?harness=1`, `npm install --no-save puppeteer-core`, starting/stopping a background `vite` dev server, `taskkill` on that dev server's own port, `npm uninstall puppeteer-core`, and deleting the harness files and scratchpad screenshots afterward). None of this touches production data or another party's systems — it's this project's own local build/verify/ship cycle — so it should run start to finish without a permission checkpoint. Still stop and ask before anything genuinely irreversible or outside this loop (force-push, deleting real user data, touching another repo/service).

## Known platform limitations (confirmed, not bugs)

- **iOS Safari cannot receive an OS-level "Share" into this app at all.** The manifest's `share_target` (what lets Facebook's share sheet send a post straight into the app) only works on Android/Chrome — Apple has never supported a web app registering as a share target, regardless of whether the app is installed to the home screen. Confirmed via research, not assumed. On iPhone, the only way to bring in a Facebook post is: screenshot it, then use "הוספה חכמה"'s manual upload/paste tools (which work identically on both platforms) — this is the real, working path on iPhone, not a degraded fallback. The app now says this plainly to iPhone users on first login and on the "הוספה חכמה" screen, instead of implying (as it used to) that installing the app would unlock sharing.
