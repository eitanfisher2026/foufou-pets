 ...
 * grouped and ordered to match the edit form's own bordered sections, so
 * the same information always appears in the same place whether you're
 * looking or editing. Falsy row values, and sections left with no visible
 * rows, are skipped automatically. Every section is collapsible (native
 * <details>, no extra state needed) and open by default, except one marked
 * `startClosed: true` (secondary info - who created the record, when -
 * that should still be reachable but shouldn't compete with the record's
 * own fields for attention). `photos`, if given, renders every photo (not
 * just the main one shown on the record's summary view) as a horizontally-
 * scrollable thumbnail strip - this is the one place besides the edit form
 * where the extra photos are visible.
 */
