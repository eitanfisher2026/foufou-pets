 ...
 * `sections` is [{ title, rows: [{ label, value }], collapsible? }] -
 ...
 * rows, are skipped automatically. A section marked `collapsible: true`
 * renders closed by default (native <details>, no extra state needed) -
 * for secondary info (who created the record, when) that should still be
 * reachable but shouldn't compete with the record's own fields for
 * attention. `photos`, if given, renders every photo (not just the main
        ...
        {visibleSections.length === 0 ? (
          ...
          <div className="space-y-4">
            {visibleSections.map((s) =>
              s.collapsible ? (
                <details key={s.title} className="rounded-xl border border-dotted border-slate-400 p-3">
