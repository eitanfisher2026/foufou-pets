...
export default function FoundReportDetail() {
  ...
  async function handleFixSpecies() {
    const newSpecies = report.species === SPECIES.DOG ? SPECIES.CAT : SPECIES.DOG;
    const ok = await confirm(
      ...
      {!showEditForm ? (
        ...
        <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-4">
          ...
          <p className="text-xs text-slate-400">
            סוג החיה סומן בטעות ({SPECIES_LABELS[report.species]})?{' '}
            <button type="button" onClick={handleFixSpecies} className="underline">
              שינוי ל{SPECIES_LABELS[report.species === SPECIES.DOG ? SPECIES.CAT : SPECIES.DOG]}
            </button>
          </p>
