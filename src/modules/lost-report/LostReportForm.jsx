...
export default function LostReportForm() {
  ...
  async function handleAnalyze() {
    ...
    try {
      const extracted = await read([mainSource], postText, fields.species);
      setFields((prev) => mergeExtractedLostFields(extracted, prev));

      const mainPhoto = await extractMainPhoto([mainSource], extracted.mainPhotoRegion);
      if (mainPhoto) {
    ...
    <form onSubmit={handleSubmit} className="space-y-5 p-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <InfoButton title={labels.infoButtonTitle}>
          <p>כמה דרכים להוסיף מידע, גם ביחד - ואז ללחוץ "זיהוי אוטומטי":</p>
          <ul className="list-inside list-disc space-y-1">
            <li>צילום מסך של הפוסט - ממלא חלק מהשדות אוטומטית.</li>
            <li>הדבקת טקסט הפוסט בתיבה - עוזר כשהכיתוב נחתך בצילום ("...עוד").</li>
            <li>הדבקת קישור לפוסט - כפתור למשיכת הטקסט והתמונה ישירות (בפוסטים פומביים בלבד).</li>
            <li>הדבקת תמונה ישירות לתיבה (Ctrl+V).</li>
            <li>אפשר להעלות כמה תמונות, אבל נסרקת רק ה"ראשית" - כדאי לוודא שהברורה ביותר מסומנת ככה.</li>
          </ul>
        </InfoButton>
      </div>
      ...
      <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4">
        ...
        {/* One upload area, not two - a photo added here (by picking or
            ...
            case" and "eligible to become the main photo", instead of the
